...
class RedMarker < Marker
    ...
    def check_additional_jump(board, current_state, current_position, jumps)
        ...
        if next_row_index >= 1 then next_row = board[next_row_index] end
        if next_row && diagonal_left_index >= 1 then diagonal_left_cell = next_row[diagonal_left_index] end
        if next_row && diagonal_right_index <= 6 then diagonal_right_cell = next_row[diagonal_right_index] end
        
        # Check if diagonal cells contain an opposite color marker
        if diagonal_right_cell && current_state[diagonal_right_cell]
            if current_state[diagonal_right_cell].color == "blue"
        ...
        if diagonal_left_cell && current_state[diagonal_left_cell]
            if current_state[diagonal_left_cell].color == "blue"
                jump_row_index = next_row_index - 1
